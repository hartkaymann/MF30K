...
public class FrontController {
	...
	public Player getPlayer(@PathVariable(name = "playerID") String name) {
		...
		System.out.println("Getting player: " + player_mgr.getPlayer(name));
		return player_mgr.getPlayer(name);

		/*
		 * HashMap<Integer, Equipment> equip = new HashMap<Integer, Equipment>();
		 * HashMap<Integer, Card> hand = new HashMap<Integer, Card>(); HashMap<Integer,
		 * Card> backPack = new HashMap<Integer, Card>(); Equipment helmet = new
		 * Equipment("testHelmet", 1, 2, equipmentType.HELMET, 123);
		 * equip.put(helmet.getId(), helmet); Player dummy = new Player("DummyPlayer",
		 * equip, Profession.BARBARIAN, Race.DWARF, Gender.FEMALE, hand, backPack, 2, 4
		 * ); return dummy;
		 */
	}
